...
function goPage(p, el) {
  ...
  if(p==='home')document.body.classList.remove('has-sidebar');else document.body.classList.add('has-sidebar');if(p==='browse')loadBrowseMakes();if(p==='market')loadMarketPage();
}
...
function loadBrowseMakes(){
  ...
  fetch(url).then(function(r){return r.json()}).then(function(d){
    ...
    var html='';
    for(var i=0;i<d.makes.length;i++){var m=d.makes[i];html+='<div class="make-card" onclick="selectMake(''+m.make+'')"><div style="font-weight:600">'+m.make+'</div><div style="font-size:0.72rem;color:var(--muted)">'+m.model_count+' models, '+m.listing_count+' listings</div></div>';}
    document.getElementById('browse-makes-grid').innerHTML=html;
...
function selectMake(mk){
  ...
  fetch(url).then(function(r){return r.json()}).then(function(d){
    ...
    var html='';
    for(var i=0;i<d.models.length;i++){var m=d.models[i];html+='<div class="row-link" onclick="selectModel(''+mk+'',''+m.model+'')"><div><div style="font-weight:600">'+m.model+'</div><div style="font-size:0.8rem;color:var(--muted)">'+m.year_range+'</div></div><div style="font-weight:600;color:var(--gold-dark)">'+m.listing_count+'</div></div>';}
    document.getElementById('browse-models-list').innerHTML=html;
...
function selectModel(mk,md){
  ...
  fetch(url).then(function(r){return r.json()}).then(function(d){
    ...
    var html='';
    for(var i=0;i<d.years.length;i++){var y=d.years[i];html+='<div class="row-link"><div><div style="font-weight:600">'+y.year+'</div><div style="font-size:0.8rem;color:var(--muted)">'+(y.trims&&y.trims.length?y.trims.join(', '):'Standard')+'</div></div><div style="font-weight:600;color:var(--gold-dark)">'+y.listing_count+'</div></div>';}
